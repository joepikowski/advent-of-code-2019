/* Solution */
function findClosestIntersection(wires, maxDistance){
    const paths = {};
    for (const [i, wire] of wires.entries()){
        paths[i] = drawWire(wire);
    }
    for (let i = 1; i <= maxDistance; i++){
        if (paths[0][i] && paths[1][i] && arraysOverlap(paths[0][i],paths[1][i])){
            return i;
        }
    }
    return false;
...
function drawWire(wire){
    ...
    function storeResult(manhattan, x, y){
        if (result[manhattan]){
            result[manhattan].push([x, y]);
        }else{
...
console.log( findClosestIntersection([wireOne, wireTwo], 5000) ); //TODO: Implement dynamic maximum calculation
