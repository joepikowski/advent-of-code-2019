/* Solution */
function findShortestDistanceToIntersection(wires, maxDistance){
    const paths = {};
    let shortestDistance = false;
    let shortestResult = [];
    let currentResult;
    for (const [i, wire] of wires.entries()){
        paths[i] = drawWire(wire);
    }
    for (let i = 1; i <= maxDistance; i++){
        if (paths[0][i] && paths[1][i]){
            currentResult = getLowestDistanceOverlap(paths[0][i],paths[1][i]);
            if (currentResult && (!shortestDistance || currentResult[2] < shortestDistance)){
                shortestDistance = currentResult[2];
                shortestResult = currentResult;
            }
        }
    }
    return shortestDistance ? shortestDistance : false;
}

function getLowestDistanceOverlap(a, b){
    let shortestDistance = false;
    let shortestResult = [];
    for (let i = 0; i < a.length; i++) {
        for (let j = 0; j < b.length; j++){
            if (a[i][0] === b[j][0] && a[i][1] === b[j][1]){
                if (!shortestDistance || a[i][2] + b[j][2] < shortestDistance){
                    shortestDistance = a[i][2] + b[j][2];
                    shortestResult = [a[i][0], a[i][1], shortestDistance];
                }
            }
        }
    }
    return shortestDistance ? shortestResult : false;
}
...
function drawWire(wire){
    ...
    for (const line of a){
        ...
        switch(direction){
            case 'U':
                while (y < distance + startY){
                    ++totalDistance;
                    manhattan = Math.abs(x) + Math.abs(++y);
                    storeResult(manhattan, x, y, totalDistance);
                }
                break;
            case 'D':
                while (y > startY - distance){
                    ++totalDistance;
                    manhattan = Math.abs(x) + Math.abs(--y);
                    storeResult(manhattan, x, y, totalDistance);
                }
    ...
    function storeResult(manhattan, x, y, dist){
        if (result[manhattan]){
            result[manhattan].push([x, y, dist]);
        }else{
            result[manhattan] = [[x, y, dist]];
        }
    }
...
console.log( findShortestDistanceToIntersection([wireOne, wireTwo], 20000) ); //TODO: Implement dynamic maximum calculation
